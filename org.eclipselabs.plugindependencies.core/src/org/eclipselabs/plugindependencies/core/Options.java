...
enum Options {
    ...
    IgnoreCycles("-ignoreCycles", false) {
        @Override
        int handle(CommandLineInterpreter cli, List<String> args) {
            if(args.isEmpty()) {
                String message = "Ignore cycles requires at least one bundle id";
                Logging.getLogger().error(message);
                return RC_RUNTIME_ERROR;
            }
            cli.setBundlesWithCycles(args);
            return RC_OK;
        }

        @Override
        void printHelp(String arg) {
            String help = "-ignoreCycles bundleId [bundleId ...]" + "\t\t\t"
                    + "Ignore cycles generated by given list of bundles."
                    + " Default is unset.";
            Logging.writeStandardOut(help);
        }
    },

    ContinueOnFail("-continueOnFail", false) {
        @Override
        int handle(CommandLineInterpreter cli, List<String> args) {
            if(!args.isEmpty()) {
                String message = "Continue on fail does not require any arguments: " + args;
                Logging.getLogger().error(message);
                return RC_RUNTIME_ERROR;
            }
            cli.setContinueOnFail(true);
            return RC_OK;
        }

        @Override
        void printHelp(String arg) {
            String help = "-continueOnFail" + "\t\t\t"
                    + "If multiple commands are used, process all of them even if some are failing."
                    + " Default is unset.";
            Logging.writeStandardOut(help);
        }
    },
